...
var[VAR::U_GT0] = pow(a_spin,2) * xbar * rho_bar_sqr * pow(CC,-4.0/3.0)/3.0 / pow(Sigma_bar,2) * ( 2 * pow(sin_theta,2) * sqrt( Delta_bar ) * ( rho_bar_sqr * ( f_0 - mass*rbar ) + 2*f_0*rbar*(2*mass*f_0) ) + 2*pow(cos_theta,2) * (rho_bar_sqr*Delta_bar - 2 *Sigma_bar ) - 3*rho_bar_sqr*(rho_bar_sqr + 2*mass*f_0*rbar) ) ;

var[VAR::U_GT1] = pow(a_spin,2) * ybar * rho_bar_sqr * pow(CC,-4.0/3.0)/3.0 / pow(Sigma_bar,2) * ( 2 * pow(sin_theta,2) * sqrt( Delta_bar ) * ( rho_bar_sqr * ( f_0 - mass*rbar ) + 2*f_0*rbar*(2*mass*f_0) ) + 2*pow(cos_theta,2) * (rho_bar_sqr*Delta_bar - 2 *Sigma_bar ) - 3*rho_bar_sqr*(rho_bar_sqr + 2*mass*f_0*rbar) ) ;

var[VAR::U_GT2] = 2*pow(a_spin,2) * zbar * pow(sin_theta,2) * rho_bar_sqr * pow(CC,-4.0/3.0)/3.0 / pow(Sigma_bar,2) * ( sqrt( Delta_bar ) * ( rho_bar_sqr * ( f_0 - mass*rbar ) + 2*f_0*rbar*(2*mass*f_0) ) - (rho_bar_sqr*Delta_bar - 2 *Sigma_bar ) ) ;
...
var[VAR::U_SYMAT0] = 2*a_spin*xbar*ybar*rbar/rho_bar_sqr/pow(Sigma_bar*rho_bar_sqr,5.0/6.0) * ( mass*rho_bar_sqr*(pow(f_0,2) + pow(a_spin*rbar,2)) - f_0*(2*mass*f_0)*(rho_bar_sqr+pow(f_0,2) + pow(a_spin*rbar,2) ) + pow(a_spin*zbar,2)*sqrt(Delta_bar)*2*mass*f_0 ) ;

var[VAR::U_SYMAT1] = - a_spin*(xbar*xbar - ybar*ybar)*rbar/rho_bar_sqr/pow(Sigma_bar*rho_bar_sqr,5.0/6.0) * ( mass*rho_bar_sqr*(pow(f_0,2) + pow(a_spin*rbar,2)) - f_0*(2*mass*f_0)*(rho_bar_sqr+pow(f_0,2) + pow(a_spin*rbar,2) ) + pow(a_spin*zbar,2)*sqrt(Delta_bar)*2*mass*f_0 ) ;

var[VAR::U_SYMAT2] = a_spin*ybar*zbar*rbar/rho_bar_sqr/pow(Sigma_bar*rho_bar_sqr,5.0/6.0) * ( mass*rho_bar_sqr*(pow(f_0,2) + pow(a_spin*rbar,2)) - f_0*(2*mass*f_0)*(rho_bar_sqr+pow(f_0,2) + pow(a_spin*rbar,2) ) - pow(a_spin,2)*(xbar*xbar+ybar*ybar)*sqrt(Delta_bar)*2*mass*f_0 ) ;

var[VAR::U_SYMAT3] = - var[VAR::U_SYMAT0] ;

var[VAR::U_SYMAT3] = - a_spin*xbar*zbar*rbar/rho_bar_sqr/pow(Sigma_bar*rho_bar_sqr,5.0/6.0) * ( mass*rho_bar_sqr*(pow(f_0,2) + pow(a_spin*rbar,2)) - f_0*(2*mass*f_0)*(rho_bar_sqr+pow(f_0,2) + pow(a_spin*rbar,2) ) - pow(a_spin,2)*(xbar*xbar+ybar*ybar)*sqrt(Delta_bar)*2*mass*f_0 ) ;
