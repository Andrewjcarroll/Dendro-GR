...
void initCompactFD(int dtype, const unsigned int nd)
{
    ...
    if (dtype == 0)
    {
        // just do the normal FD
    }
    else if (dtype == 1)
    {
        if (initKimDeriv4(R, nd))
        {
            std::cerr << "Could not construct Kim Deriv4 matrix!!!"
                      << std::endl;
        }
    }
    else if (dtype == 2)
    {
        if (initHAMRDeriv4(R, nd))
        {
            std::cerr << "Could not construct HAMR Deriv4 matrix!!!"
                      << std::endl;
        }
    }
    else if (dtype == 3)
    {
        if (initJTPDeriv6(R, nd))
        {
            std::cerr << "Could not construct HAMR Deriv4 matrix!!!"
                      << std::endl;
        }
    ...
    {
        std::cerr << "initCompactFD:  Unknown dtype = " << dtype << std::endl;
    }
}
void init_filter_CompactFD(int ftype, const unsigned int nd)
{
    ...
    {
        if (initKim_Filter_Deriv4(RF, nd))
        {
            std::cerr << "Could not construct Kim Deriv4 filter matrix!!!"
                      << std::endl;
        }
    }
    ...
    {
        std::cerr << "init_filter_CompactFD:  Unknown ftype = " << ftype << std::endl;
    }
}
/*----------------------------------------------------------------------;
 *
...
void cfd_x(double *const Dxu, const double *const u, const double dx,
           const unsigned int *sz, double *const D, double *const f1d,
           double *const df1d, unsigned bflag)
{
    const int nx = sz[0];
    const int ny = sz[1];
    const int nz = sz[2];
    ...
    for (unsigned int k = 0; k < nz; k++)
    {
        for (unsigned int j = 0; j < ny; j++)
        {
            ...
            for (unsigned int i = 0; i < nx; i++)
            {
                ...
                {
                    df1d[i] += D[i * nx + m] * f1d[m];
                }
            }

            for (unsigned int i = 0; i < nx; i++)
            {
                Dxu[IDX(i, j, k)] = df1d[i] / dx;
            }
        }
    }
}
...
void cfd_y(double *const Dyu, const double *const u, const double dy,
           const unsigned int *sz, double *D, double *const f1d,
           double *const df1d, unsigned bflag)
{
    const int nx = sz[0];
    const int ny = sz[1];
    ...
    for (unsigned int k = 0; k < nz; k++)
    {
        for (unsigned int i = 0; i < nx; i++)
        {
            ...
            for (unsigned int j = 0; j < ny; j++)
            {
                ...
                {
                    df1d[j] += D[j * ny + m] * f1d[m];
                }
            }
            for (unsigned int j = 0; j < ny; j++)
            {
                Dyu[IDX(i, j, k)] = df1d[j] / dy;
            }
        }
    }
}
    ...
    for (unsigned int j = 0; j < ny; j++)
    {
        for (unsigned int i = 0; i < nx; i++)
        {
            ...
            for (unsigned int k = 0; k < nz; k++)
            {
                ...
                {
                    df1d[k] += D[k * nz + m] * f1d[m];
                }
            }
            for (unsigned int k = 0; k < nz; k++)
            {
                Dzu[IDX(i, j, k)] = df1d[k] / dz;
            }
        }
    }
}
...
void filter_cfd_x(double *const u, const double dx, const unsigned int *sz, double *RF, double *f1d, double *df1d, unsigned bflag)
    ...
    for (unsigned int k = 0; k < nz; k++)
    {
        for (unsigned int j = 0; j < ny; j++)
        {
            ...
            for (unsigned int i = 0; i < nx; i++)
            {
                ...
                for (unsigned int m = 0; m < nx; m++)
                {
                    df1d[i] += RF[i * nx + m] * f1d[m];
                }
            }

            for (unsigned int i = 0; i < nx; i++)
            {
                u[IDX(i, j, k)] += df1d[i]; // do we need to include a dx here?
            }
        }
    }
}
 ...
 *
 *----------------------------------------------------------------------*/
void filter_cfd_y(double *const u, const double dx, const unsigned int *sz, double *RF, double *f1d, double *df1d, unsigned bflag)
    ...
    for (unsigned int k = 0; k < nz; k++)
    {
        for (unsigned int i = 0; i < nx; i++)
        {
            ...
            for (unsigned int j = 0; j < ny; j++)
            {
                ...
                for (unsigned int m = 0; m < ny; m++)
                {
                    df1d[j] += RF[j * ny + m] * f1d[m];
                }
            }
            for (unsigned int j = 0; j < ny; j++)
            {
                u[IDX(i, j, k)] += df1d[j]; // do we need to include a dy here?
            }
        }
    }
}
        ...
        for (unsigned int j = 0; j < ny; j++)
    {
        for (unsigned int i = 0; i < nx; i++)
        {
            ...
            for (unsigned int k = 0; k < nz; k++)
            {
                ...
                for (unsigned int m = 0; m < nz; m++)
                {
                    df1d[k] += RF[k * nz + m] * f1d[m];
                }
            }
            for (unsigned int k = 0; k < nz; k++)
            {
                u[IDX(i, j, k)] += df1d[k]; // do we need to include a dz here?
            }
        }
    }
